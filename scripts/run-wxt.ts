...
const args = localeFlagIndex >= 0
  ? argv.filter((_, index) => index !== localeFlagIndex && index !== localeFlagIndex + 1)
...
const child = spawn(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['exec', 'wxt', '--', ...args], {
  stdio: 'inherit',
  env: {
    ...process.env,
